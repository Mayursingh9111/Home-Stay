...
const ListRoom = () => {
  ...
  // Fetch all rooms for the owner (GET request)
  ...
  // Toggle room availability (POST request)
  const toggleAvailability = async (roomId) => {
    try {
      ...
      if (data.success) {
        ...
        // ✅ Update only that room in local state without full refetch
        setRooms((prevRooms) =>
          prevRooms.map((room) =>
          ...
          <tbody className="text-sm">
            {rooms.map((item) => (
              <tr key={item._id}>
                ...
                <td className="py-3 px-4 border-gray-300 border-t text-sm text-center">
                  {/* ✅ FIXED: Proper structure for toggle */}
